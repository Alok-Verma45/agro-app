...
function AdminProducts() {
  ...
  const handleSave = async () => {
    if (!form.name || !form.price) {
      showToast("⚠️ Name & Price required");
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        if (!form.image) {
          showToast("⚠️ Product image required");
          return;
        }
      ...
      if (message === "Product already exists") {
        showToast("⚠️ Product already exists");
      } else {
  ...
  return (
    <div className="py-6 space-y-6">
      <input
        ...
        className="w-full p-4 rounded-xl bg-white dark:bg-white/5 border border-white/10 text-gray-900 dark:text-white outline-none"
        value={search}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredProducts.map((p) => (
              <div
                key={p.id}
                className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 p-4 rounded-xl bg-white/5"
              >
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              {(form.preview || form.oldImage) && (
                <img
                  src={form.preview || form.oldImage}
                  alt="Preview"
